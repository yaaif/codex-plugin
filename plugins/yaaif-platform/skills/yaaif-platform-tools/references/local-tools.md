...
Built-in tools live in agent-service (`internal/mcp/local_tools.go`). Codex discovers them via `GET /api/local-tools` (`yaaif_local_tools_list`).
...
## Codex REST vs local tools

| Goal | Prefer |
|------|--------|
| Browse catalog / enable / map agents | Codex REST: `yaaif_skill_*`, `yaaif_agent_*` |
...
| Approval strategy CRUD | Codex REST `yaaif_approval_strategy_*` |
| Approval inbox decide at runtime | Local `yaaif_approval_*` or Codex ambient/approval ops tools |
...
- Codex REST helpers (outside local tools): `yaaif_file_artifact_versions`,
