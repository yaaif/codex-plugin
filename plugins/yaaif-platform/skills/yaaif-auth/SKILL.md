...
name: yaaif-auth
description: >-
  Authenticate to YAA\F from Codex, select platform profile and tenant, and
...
Active profile is stored in `~/.yaaif/codex/active-profile.json`. Switching
...
- Session file: `~/.yaaif/codex/session.json` (includes `profile_id` + `oidc_authority`).
