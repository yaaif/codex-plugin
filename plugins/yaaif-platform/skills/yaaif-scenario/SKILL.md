...
description: >-
  ...
  objects, sync objects from the spec, or Admin UI Open in Codex selected a
  spec_id.
---
...
record for spec-driven development. Codex plugin creates/updates bind with
...
## Mode A — selected spec (Admin UI / Open in Codex)

When the prompt includes `spec_id` (and usually `slug` / `name`):

1. Stay in Codex. Do **not** open Admin UI or browser URLs for the scenario.
...
3. Direction (always preview before applying):
   - **from objects** (default): live catalog → spec
     (`yaaif_agent_spec_sync_preview`, then `yaaif_agent_spec_sync_apply`). Use after Codex plugin changes to
